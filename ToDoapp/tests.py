...
class TestProjectViewSet(TestCase):
    ...
    def test_create_admin(self):
        ...
        request = factory.post(
            "/api/projects/",
            {"name": "New project", "repository": "http://somelink.ru", "users": [admin.uuid]},
            format="json",
        )
...
class TestTODOViewSet(APITestCase):
    ...
    def test_edit_admin(self):
        admin = User.objects.create_superuser("admin", "admin@admin.com", "admin123456")
        project = Project.objects.create(name="New project", repository="http://somelink.ru")
        todo = TODO.objects.create(project=project, creator=admin, text="some text")
        self.client.login(username="admin", password="admin123456")
        response = self.client.put(
            f"/api/todos/{todo.id}/",
            {"project": project.id, "creator": admin.uuid, "text": "some text"},
        )
